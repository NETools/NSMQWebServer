...
namespace NSMQWebServer.Persistence
{
	public class MessageQueueDbContext : DbContext
	{
		...
		public async Task<User?> FindUser(Guid userId) => await Users
			.Include(p => p.TaskDatas)
			.Where(p => p.UserId == userId)
			.FirstOrDefaultAsync();

		public async Task<Channel?> FindChannel(string channelId) => await Channels
			.Include(p => p.Users)
			.ThenInclude(p => p.TaskDatas)
			.Where(p => p.ChannelId == channelId)
			.FirstOrDefaultAsync();

		public IQueryable<Channel> FindChannelQuery(string channelId) => Channels
			.Include(p => p.Users)
			.ThenInclude(p=>p.TaskDatas)
			.Where(p => p.ChannelId == channelId);

		public async Task<TaskData?> FindTask(Guid taskId) => await TaskDatas
			.Where(p => p.TaskId == taskId)
			.FirstOrDefaultAsync();

		public async Task<TaskData?> FindTaskPhased(Guid phaseId) => await TaskDatas
		...
		{
			await FindChannelQuery(channelId).SelectMany(p => p.Users, (channel, user) => new { Channel = channel, User = user })
				.Where(p => p.User != null && p.User.UserType == userType).ForEachAsync(p => action(p.Channel, p.User));
		}
